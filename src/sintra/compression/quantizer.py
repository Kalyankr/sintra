...
def check_llama_cpp_available() -> Tuple[bool, str]:
    ...
    quantize_found = any(
        p and (Path(p).exists() if isinstance(p, str) else p.exists())
...
class GGUFQuantizer:
    ...
    def convert_to_gguf(
        ...
        try:
            ...
            if result.returncode != 0:
                raise QuantizationError(f"GGUF conversion failed:\n{result.stderr}")
    ...
    def quantize(
        ...
        output_file = (
            self.quantized_dir / f"{model_name}-{quant_type.value.lower()}.gguf"
        )
        if output_file.exists():
    ...
    def _run_quantize(
        ...
        try:
            ...
            if result.returncode != 0:
                raise QuantizationError(f"Quantization failed:\n{result.stderr}")
    ...
    def quantize_with_compression(
        ...
        output_file = (
            self.quantized_dir / f"{full_name}-{quant_type.value.lower()}.gguf"
        )
