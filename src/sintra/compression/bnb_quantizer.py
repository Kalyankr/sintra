...
class BitsAndBytesQuantizer:
    ...
    def quantize(
        ...
        sys.stderr.write(f"BnB: Loading {model_id_or_path} with {bits}-bit {quant_type.value}...\n")
        ...
        try:
            ...
            if output_name is None:
                model_name = Path(model_id_or_path).name if "/" not in model_id_or_path else model_id_or_path.split("/")[-1]
