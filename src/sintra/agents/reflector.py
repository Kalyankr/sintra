...
class StrategyAdjustment(BaseModel):
    ...
    parameter: str = Field(description="Which parameter to adjust: bits, pruning, layers")
...
def reflector_node(state: SintraState) -> Dict[str, Any]:
    ...
    if _all_targets_met(last_metrics, profile):
        log_transition("Reflector", "Last attempt successful, no reflection needed", "status.success")
        return {"reflection": None}
...
def reflector_node_llm(state: SintraState) -> Dict[str, Any]:
    ...
    try:
        ...
        response = llm.invoke([
            ...
            {"role": "user", "content": "Analyze the attempts and provide recommendations."},
        ])
...
def _analyze_history(
    history: List[Dict], profile, state: SintraState
) -> Reflection:
    ...
    try:
        ...
        if json_match:
            ...
            return Reflection(
                ...
                failures=[
                    FailureAnalysis(**f) for f in data.get("failures", [])
                ],
