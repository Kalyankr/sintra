    ...
    for key, value in state.items():
        if key == "profile" and isinstance(value, HardwareProfile):
            ...
            if isinstance(value, dict):
                serialized[key] = {
                    "recipe": value["recipe"].model_dump()
                    if hasattr(value["recipe"], "model_dump")
                    else value["recipe"],
                    "metrics": {
                    ...
                    if hasattr(value["metrics"], "actual_tps")
                    else value["metrics"],
                }
            else:
            ...
            for entry in value:
                if isinstance(entry, dict):
                    serialized_entry = {
                        "recipe": entry["recipe"].model_dump()
                        if hasattr(entry["recipe"], "model_dump")
                        else entry["recipe"],
                        "metrics": {
                        ...
                        if hasattr(entry["metrics"], "actual_tps")
                        else entry["metrics"],
                    }
