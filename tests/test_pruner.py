...
class TestLayerDropper:
    ...
    def test_extract_layer_index_llama(self) -> None:
        ...
        assert dropper._extract_layer_index("model.layers.0.self_attn.q_proj.weight") == 0
        assert dropper._extract_layer_index("model.layers.15.mlp.gate_proj.weight") == 15
        assert dropper._extract_layer_index("model.layers.31.input_layernorm.weight") == 31
    ...
    def test_extract_layer_index_bert(self) -> None:
        ...
        assert dropper._extract_layer_index("encoder.layer.0.attention.self.query.weight") == 0
        assert dropper._extract_layer_index("encoder.layer.5.intermediate.dense.weight") == 5
    ...
    def test_replace_layer_index(self) -> None:
        ...
        # Llama style
        result = dropper._replace_layer_index("model.layers.5.self_attn.q_proj.weight", 5, 3)
        assert result == "model.layers.3.self_attn.q_proj.weight"
        
        # GPT-2 style
        result = dropper._replace_layer_index("transformer.h.10.attn.c_attn.weight", 10, 8)
...
class TestStructuredPruner:
    ...
    def test_should_prune_linear_weights(self) -> None:
        ...
        # Should prune
        assert pruner._should_prune("model.layers.0.self_attn.q_proj.weight", torch.randn(256, 256))
        assert pruner._should_prune("model.layers.0.mlp.gate_proj.weight", torch.randn(512, 256))
        assert pruner._should_prune("transformer.h.0.attn.c_attn.weight", torch.randn(768, 2304))

    def test_should_prune_skip_embeddings(self) -> None:
        """Test _should_prune skips embeddings."""
        pruner = StructuredPruner()
        
        assert not pruner._should_prune("model.embed_tokens.weight", torch.randn(32000, 768))
    ...
    def test_should_prune_skip_norms(self) -> None:
        ...
        assert not pruner._should_prune("model.layers.0.input_layernorm.weight", torch.randn(768))
    ...
    def test_should_prune_skip_1d(self) -> None:
        ...
        assert not pruner._should_prune("model.layers.0.self_attn.q_proj.bias", torch.randn(256))
...
class TestConvenienceFunctions:
    ...
    def test_apply_compression_no_ops(self, tmp_path: Path) -> None:
        ...
        result = apply_compression(model_dir, pruning_ratio=0.0, layers_to_drop=None, cache_dir=tmp_path)
        assert result == model_dir
...
class TestIntegration:
    ...
    def test_layer_dropper_creates_output_name(self, mock_model_dir: Path, tmp_path: Path) -> None:
