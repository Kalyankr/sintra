...
class TestBitsAndBytesQuantizer:
    ...
    def test_bnb_quantizer_raises_without_deps(self):
        ...
        from sintra.compression.bnb_quantizer import BitsAndBytesError, is_bnb_available
...
class TestBnBQuantizerUnit:
    ...
    def mock_bnb_available(self):
        ...
        with (
            patch(
                "sintra.compression.bnb_quantizer._check_bnb_available",
                return_value=True,
            ),
            patch(
                "sintra.compression.bnb_quantizer._check_cuda_available",
                return_value=True,
            ),
        ):
            yield
...
class TestONNXOptimizerUnit:
    ...
    def mock_optimum_available(self):
        ...
        with (
            patch(
                "sintra.compression.onnx_optimizer._check_optimum_available",
                return_value=True,
            ),
            patch(
                "sintra.compression.onnx_optimizer._check_onnxruntime_available",
                return_value=True,
            ),
        ):
            yield
